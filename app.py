...
def search(search_str):
    return pd.read_sql(
        f"select * from entries where ocr_text like '%{search_str}%' order by created_timestamp desc",
        con=conn,
    )
...
def show_entry(search_str, entry):
    ...
    for i in range(n_boxes):
        ...
        if search_str.lower() in ocr_i["text"].lower():
            cv2.rectangle(
                img,
                (ocr_i["left"] - 2, ocr_i["top"] - 2),
                (
                    ocr_i["left"] + ocr_i["width"] + 2,
                    ocr_i["top"] + ocr_i["height"] + 2,
                ),
                (0, 255, 0),
                4,
            )
...
st.markdown(f"<h1 style='text-align: center'>Rewind</h1>", unsafe_allow_html=True)

st.text_input("", placeholder="Search your past...", key="search_str")

if st.session_state.search_str:
    if (
        "shown_idx" not in st.session_state
        or st.session_state.last_search != st.session_state.search_str
    ):
    ...
    if len(records):
        ...
        col2.markdown(
            f"<h3 style='text-align: center'>{humanize_date_difference(entry['created_timestamp'])}</h3>",
        ...
        st.text("Search query not found")
